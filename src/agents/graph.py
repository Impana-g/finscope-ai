...
from src.agents.nodes.planner import generate_plan
from src.agents.nodes.researcher import execute_step_node
from src.agents.nodes.reporter import generate_report_node


def plan_research_node(state: dict) -> dict:
    plan = generate_plan(state["original_query"], state["sector"])
...
def build_graph():
    ...
    graph.add_conditional_edges(
        "await_approval",
        lambda s: "execute_step" if s.get("plan_approved") else END
    )
